    ...
    ILogger<ResendEmailSender> log) : IEmailSender
{
    ...
    public async Task<string> SendAsync(EmailMessage message, CancellationToken ct = default)
    {
        ...
        {
            // This triggers when the HttpClient.Timeout hits, but NOT when the user cancels
            _log.LogError(ex, "Resend API timed out.");
            throw new EmailSendException("The email provider took too long to respond.", ex);
        }
